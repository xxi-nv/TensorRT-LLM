...
class MoeBackendType(str, Enum):
    ...
    # and grep are unambiguous (see MEGAMOE_CUTEDSL_DESIGN.md "Shared
    # helpers" -> asymmetric pair anti-pattern). The legacy
...
def should_skip_megamoe_cutedsl(
    ...
    if moe_tp_size != 1:
        return f"MegaMoECuteDsl v1 is EP-only (got moe_tp_size={moe_tp_size})"
...
def supports_autotuner_capture(
    ...
    # (their fused kernels own dispatch+combine and have static compile shapes).
    if backend_type in (
