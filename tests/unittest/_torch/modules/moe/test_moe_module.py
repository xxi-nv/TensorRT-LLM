...
def generate_multi_gpu_test_params(
    ...
    for parallel_mode, comm_method in product(parallel_modes, comm_methods):
        for (
            ...
            # Check comm-method-specific skip conditions
            if not skip_reason:
                skip_reason = _get_comm_method_skip_reason(comm_method, model_config)

            # Check backend + comm-method compatibility
            if not skip_reason:
            ...
            if not skip_reason:
                skip_reason = should_skip_DEEPGEMM(backend_type, comm_method)
...
def test_moe_single_gpu(
    ...
    """
    Test MoE module with autotune to capture all tactics.

    This test verifies:
    1. Autotune works correctly with the module
