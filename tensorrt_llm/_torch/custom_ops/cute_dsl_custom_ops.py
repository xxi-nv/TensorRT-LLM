...
if IS_CUTLASS_DSL_AVAILABLE:
    ...
    class Sm100BlockScaledMegaMoeBlackwellRunner(
        ...
        def forward(self, inputs: List, tactic: Optional[tuple],
            ...
            monolithic_final_output_rank_stride_elements = int(
                kwargs.get("monolithic_final_output_rank_stride_elements", 0))
            ...
            if monolithic_reduce_output:
                ...
                assert monolithic_final_output.dim() == 3
                assert monolithic_final_output.stride(2) == 1
            ...
            if direct_combine_output:
                ...
                # ``output`` is the FC2 combine buffer. New NVLink rank-strided
                # layout passes 3D ``(ep, top_k*max_tok, hidden)`` from
                # ``MegaMoECuteDSL._alloc_nvlink_workspace``; older 2D layout
                # ``(ep, top_k*max_tok*hidden)`` stays valid for flat callers.
                assert output.dim() in (2, 3)
                assert output.size(0) >= combine_output_ep_size
                output_elements_per_rank = output.numel() // output.size(0)
                assert output_elements_per_rank >= required_output_elements_per_rank, (
                    f"direct combine output row has {output_elements_per_rank} elements, "
                    f"need {required_output_elements_per_rank}")
                assert output.stride(output.dim() - 1) == 1
                ...
                if monolithic_reduce_output:
                    ...
                    # ``monolithic_final_output`` is 3D ``(ep, max_tok, hidden)``
                    # when allocated via the NVLink rank-strided workspace.
                    assert monolithic_final_output.size(
                        1) >= monolithic_local_tokens
                    assert monolithic_final_output.size(2) == n_l2
            else:
            ...
            cache_key = (
                ...
                monolithic_control_rank_stride_elements,
                monolithic_final_output_rank_stride_elements,
                monolithic_local_rank, monolithic_direct_topk_materialize,
    ...
    def _run_cute_dsl_nvfp4_mega_moe_blackwell_with_pool(
        ...
        runner.forward(
            ...
            monolithic_final_output_rank_stride_elements=(
                monolithic_final_output.stride(0)
                if monolithic_final_output is not None
                and monolithic_final_output.dim() >= 3 else 0),
            monolithic_local_rank=monolithic_local_rank,
