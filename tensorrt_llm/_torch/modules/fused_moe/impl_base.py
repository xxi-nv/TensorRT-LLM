...
def apply_moe_impl_construction_state(
    ...
    *,
    ...
    """Install the construction state backends used to read off ``MoE``.

    ``MoEImplBase`` does not inherit ``MoE`` and therefore does not run
    ``MoE.__init__``. The attributes below are the ones every execution-unit
    implementation still reads (``hidden_size``, ``quant_config``, ``mapping``,
    partition sizes, and the default EPLB layout the wrapper later overwrites).

    Layer-only work is intentionally omitted: ``_register_layer``, the full
    ``_init_load_balancer``, ``AllReduce``, DWDP layout, and the perfect
    router. Those stay on ``ConfigurableMoE`` / ``MoE``.

    ``init_load_balancer=True`` is the old standalone-layer path. It is a
    hard failure here because these classes no longer implement ``forward``.
    The wrapper always passes ``False``.
    """
    ...
    # Complete-layer AllReduce lives on the wrapper. Keep the attribute so
    # leftover getattr sites do not grow a second meaning.
...
class MoEImplBase(
    ...
    ``eplb`` is optional during the base-class swap. While it is ``None``,
    ``apply_moe_impl_construction_state`` installs the same default layout
    ``MoE.__init__(init_load_balancer=False)`` used to, and the wrapper's
    ``_BACKEND_SYNC_ATTRS`` snapshot keeps working. A later EPLB item makes
    ...
    # The rest of the scheduler-facing defaults come from
    # ``MoEExecutionContractMixin``, shared with ``MoE``. Only ``scheduler_kind``
    # is restated: its default needs ``MoESchedulerKind`` from ``interface``,
    # which the mixin's module cannot import without a cycle.
    ...
    # ---- weight lifecycle -------------------------------------------------
    # ``create_weights`` / ``load_weights`` / ``_check_configs`` come from
    # ``MoEWeightOwnerMixin`` as working defaults, not as abstract markers: the
    # allocate-then-delegate-to-quant-method shape is the same for most backends,
    # and the ones that need more (TRTLLMGen, MegaMoE-CuteDsl) override.
    # ``_get_quant_method`` has no sensible default and stays abstract below.

    @abc.abstractmethod
    def _get_quant_method(self) -> object:
        """Resolve the quantization method that owns this backend's weights.

        Abstract because the returned methods are backend-family specific -- a
        Cutlass-layout NVFP4 method is not interchangeable with a TRTLLMGen one,
    ...
    # ``validate_configurable_moe`` comes from ``MoEExecutionContractMixin``.
