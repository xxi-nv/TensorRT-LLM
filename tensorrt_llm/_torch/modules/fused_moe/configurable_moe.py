...
class ConfigurableMoE(MoE):
    ...
    def _get_nvlink_onesided_moe_output(
        ...
        if all_rank_num_tokens is not None:
            ...
            # Fallback: this shouldn't happen in normal flow but provide a safe default
            logger.warning(
                "all_rank_num_tokens not provided for NVLinkOneSided backend, "
                "this may cause incorrect buffer sizing"
            )
            runtime_max_tokens_per_rank = 0
    ...
    def _get_backend_kwargs(
        ...
        if self.backend.__class__ == CutlassFusedMoE:
            ...
            supports_post_quant = (
                self.comm is not None and self.comm.supports_post_quant_dispatch()
            )
            ...
            if self._is_using_alltoall():
                if all_rank_num_tokens is not None:
                    ...
                    kwargs["tuner_num_tokens"] = x.shape[0] * self.mapping.tp_size if x is not None else None
