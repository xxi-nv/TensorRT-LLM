...
class ConfigurableMoE(MoE):
    ...
    Auto-Detection:
        ...
        - Communication: Auto-selected based on hardware (MNNVL > DeepEP > AllGather)
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Call create_moe with all necessary parameters
        self.backend = create_moe(
    ...
    def _is_using_alltoall(self) -> bool:
        ...
        Returns:
            True: Strategy uses alltoall (MNNVL, DeepEP, etc.)
    ...
    def _forward_chunk_impl(
        ...
        # ========== Step 2: Apply routing (only if backend supports separation) ==========

        if self.backend.supports_routing_separation():
            ...
            assert not self._using_dynamic_load_balancer(), (
        ...
        if self.layer_load_balancer and token_selected_experts is not None:
            ...
            # Update EPLB statistics (method depends on whether using MNNVL)
            # Use base class method: ignore_allreduce=True for MNNVL (uses local stats only)
            ignore_allreduce = self._is_using_mnnvl_latency()
        ...
        # MNNVL has a prepare phase to gather EPLB statistics

        # Only MNNVL needs prepare_dispatch
        if self._is_using_mnnvl_latency():
            ...
            # Call prepare_dispatch (gathers statistics for MNNVL)
        ...
        # token_selected_experts and token_selected_slots: one will be used, one will be None
        final_hidden_states = self.backend.run_moe(
            input=x,
            token_selected_experts=token_selected_slots
            if not self.backend.supports_routing_separation() or token_selected_slots is not None
            else None,
            token_selected_slots=token_selected_slots,
            token_final_scales=token_final_scales,
            input_sf=x_sf,
    ...
    def backend(self, backend: MoE):
        ...
        if self._using_dynamic_load_balancer() and not backend.supports_routing_separation():
            raise ValueError(
                f"EPLB is enabled but backend {backend.__class__.__name__} "
                f"does not support routing separation (has fused routing). "
                f"Either disable EPLB or use a backend with separated routing."
            )
    ...
    def comm(self, strategy: Optional[Communication]):
        ...
        # Validate strategy is a Communication
        if not isinstance(strategy, Communication):
            raise TypeError(f"comm must be a Communication instance, got {type(strategy).__name__}")
    ...
    def _is_using_mnnvl_latency(self) -> bool:
        """Check if using MNNVLLatency communication strategy"""
        return isinstance(self.comm, MnnvlLatency)
    ...
    def _get_backend_kwargs(self) -> Dict:
        ...
        Backend-specific kwargs:
            ...
            - TRTLLMGen: router_logits, routing_bias, do_finalize, output_tensor
    ...
    def load_weights(self, weights: List[Dict]):
        ...
        return self.backend.load_weights(self, weights)
    ...
    def post_load_weights(self):
        ...
        return self.backend.post_load_weights(self)
