...
class MoE(MoEExecutionContractMixin, MoEWeightOwnerMixin,
    ...
    # ``capabilities``, ``input_requirement``, ``_supports_non_divisible_ep``,
    # and ``supports_moe_output_in_alltoall_workspace`` come from
    # ``MoEExecutionContractMixin`` so this class and ``MoEImplBase`` cannot
    # drift apart. ``scheduler_kind`` cannot join them: its default needs
    # ``MoESchedulerKind``, which is defined in this module, and ``impl_blocks``
    # is imported *by* this module -- moving the enum to ``impl_contract`` would
    # let it move too.
    ...
    # ``create_weights`` / ``load_weights`` come from ``MoEWeightOwnerMixin``,
    # shared with ``MoEImplBase``. They used to be abstract here and abstract
    # again there, which meant two declarations of the same requirement and no
    # implementation for either -- every subclass hand-wrote the same
    # allocate-then-delegate body.
    ...
    # ``forward_fake`` comes from ``MoEExecutionContractMixin``: the output
    # shape follows from the input and ``self.mapping``, so a backend can answer
    # it too, and the backends that override it delegate the finalized case back
    # to that default via ``super()``.
    ...
    def enable_alltoall(self):
        ...
        A layer-level question: the answer follows from the communication
        strategy the layer owns, which is why this stays here and is NOT part of
        the execution-unit contract. ``ConfigurableMoE`` overrides it from
        ``self.comm``; an execution unit is never asked.
        """
        return False
