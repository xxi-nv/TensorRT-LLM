...
def get_moe_cls(
    ...
    if moe_backend.upper() == "CUTLASS":
        ...
        # SM100 family GPUs. Mirrors the MEGAMOE_DEEPGEMM fall-back pattern:
        # if the active environment cannot serve this backend (unsupported
        # quant / dtype / hidden / intermediate / SM family, or the CUDA 13
        # Cutlass DSL runtime symbols required by the kernel are missing),
        # fall back to CutlassFusedMoE so no MegaMoE-specific weight
        # tensors get allocated.
        if quant_config is None or not quant_config.quant_mode.has_nvfp4():
            logger.warning(
