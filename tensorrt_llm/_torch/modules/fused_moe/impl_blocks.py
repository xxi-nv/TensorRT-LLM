...
Three groups, by what the module is being asked about: what it declares to the
scheduler (:class:`MoEExecutionContractMixin`), what it does with the weights
(:class:`MoEWeightOwnerMixin`), and how those weights are laid out under EPLB
...
class MoEExecutionContractMixin:
    ...
    Two kinds of question live here: what the module declares statically to the
    scheduler, and what output shape it would produce for a given input. Both
    are answered by the backend rather than the layer, so both are stated once
    and shared by ``MoE`` and ``MoEImplBase`` -- a backend that moves from the
    first base to the second must keep answering them unchanged.

    Copying these defaults into each base instead is what this class exists to
    prevent. A hand-copied list omits members, and an omission here does not
    fail at construction -- it fails at the first call that asks.
    """
    ...
    def forward_fake(
        ...
        **kwargs,
        ...
        Only needs the input and ``self.mapping``, which is why it can be
        answered by an execution unit and not just by a layer. Backends whose
        output dtype or un-finalized layout differs override this and delegate
        the finalized case back here.
        """
...
class MoEWeightOwnerMixin:
    ...
    def create_weights(self) -> None:
        ...
        The shape most backends need, so it is the default rather than a
        per-backend copy. Override when allocation needs more than this:
        TRTLLMGen passes a fused-shared-expert count for FP8 block scales, and
        MegaMoE-CuteDsl must rendezvous a symmetric-memory provider first,
        before the quant method registers anything.
        """
    ...
    def _check_configs(self) -> None:
        ...
        it was built with (a top-k restriction, a quant-mode allow-list) states
