...
| `MEGAMOE_CUTEDSL_DESIGN.md` | Design contract for the CuteDSL NVFP4 backend (capability gate, NVFP4 ABI, NVSHMEM provider hard gate, tactic representation, EPLB) |
...
| `mega_moe/MEGAMOE_CUTEDSL_DESIGN.md` | MegaMoE CuteDSL NVFP4 backend design (capability gate, NVFP4 ABI, NVSHMEM provider, tactic representation, EPLB) |
...
| NVFP4 | Y (SM100/103/120/121) | Y (SM100/103) | N | Y (SM100/103) | Y (SM100/103) | N | Y (SM100/103, cu13 cutlass-dsl + NVSHMEM provider; v1 alpha=1) | N | Y | Y |
...
- Dynamic EPLB requires backend and quantization-method support. Backends gate
  ...
  `num_slots % ep_size == 0`. `MegaMoECuteDsl` declares `eplb_support_status = NOT_VERIFIED` because v1 lacks transformed-scale CPU staging buffers (post-`to_blocked`); see `mega_moe/MEGAMOE_CUTEDSL_DESIGN.md` "EPLB status".
...
- **Do NOT allocate symmetric memory from `run_moe` in `FUSED_COMM` backends** — Symmetric-memory rendezvous is a build-time collective and is unsafe under PP / layer-skip or CUDA graph capture; allocate from `create_weights()` after `ConfigurableMoE` has synchronized EPLB-derived attributes. See `mega_moe/mega_moe_deepgemm.py:_alloc_symm_buffer` for the DG pattern and `mega_moe/MEGAMOE_CUTEDSL_DESIGN.md` "Symmetric Memory Design" for the NVSHMEM-equivalent provider contract.
