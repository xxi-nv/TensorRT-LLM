...
"""Fused fc1+fc2 swap-AB SwiGLU NVFP4 MegaMoE kernel for SM100.

The current product path is inference-oriented and swap-AB.  The scheduler keeps
``is_swap_ab`` as an explicit coordinate convention so future MegaMoE training
variants can introduce a non-swap kernel without rewriting the shared base.
"""
...
from .epilogue import SwapABSwigluFp4Epilogue, _TmemTranspose16x32Core
...
# =============================================================================
# token_comm_args -- subclass-owned opaque bundle
# =============================================================================
#
# ``fc1fc2_kernel_impl`` accepts an Optional ``token_comm_args`` parameter and
# forwards it AS-IS to the ``token_comm_hook_*`` methods (see the hook block
# inside the base class).  The base never reads its fields, never imports the
# concrete class, never serializes it directly -- MLIR serialization works
# because cuTeDSL routes through whatever ``__extract_mlir_values__`` /
# ``__new_from_mlir_values__`` the concrete class provides.
#
# The hook-based design means the subclass alone decides:
#   - what fields the bundle carries (sizes view, peer_rank_ptr_mapper, sm_count,
#     world_size, etc.) and what to name them
#   - which dispatch primitives to call (mega_moe vs. a future variant)
#   - what leader id semantics to use inside ``token_comm_hook_kernel_tail``
#     (``%smid`` vs. CTA linear id vs. block_idx().0 = 0)
#   - whether to need extra SMEM (return a struct from
#     ``token_comm_extra_smem_storage_class`` or None)
#
# The lean fc1+fc2 path always passes ``None`` and the entire MegaMoE side of
# the kernel disappears at codegen time.
...
class Sm100SwapABSwigluFp4Fc12Kernel:
    ...
    See ``megamoe_design.md`` for layout, scheduler, and sync design.
    """
    ...
    def __init__(
        ...
        # v1 locks (kwargs for forward-compat with future dtypes)
        acc_dtype: Type[cutlass.Numeric] = cutlass.Float32,
        sf_vec_size: int = 16,
        scenario: Literal["2Dx3D"] = "2Dx3D",
        # MegaMoE form B opt-in.  Always False for the lean fc1+fc2 path
        # (which has no topk axis to reduce).  Plumbed through to the
        # epilogue construction in ``_setup_attributes`` so the same
        # subclass-side rejection covers both routes.
        ...
        if sf_vec_size != Nvfp4BlockSize:
            raise NotImplementedError(
                f"v1 only supports sf_vec_size={Nvfp4BlockSize} (NVFP4); got {sf_vec_size}."
            )
        if scenario != "2Dx3D":
            raise NotImplementedError(
                f"v1 fused fc12 only supports scenario='2Dx3D' (forward); got {scenario!r}."
            )
        ...
        # MegaMoE form B opt-in (epilogue currently rejects True).  Stored
        # so ``_setup_attributes`` can forward it when constructing the
        ...
        # MegaMoE toggle.  Defaults to False for the base (local fc1+fc2)
        # kernel; subclasses that fuse dispatch / combine flip this flag
        # in their own ``__init__`` after calling ``super().__init__``.
        # The flag is FIRST READ inside ``_setup_attributes`` to decide
        # warp topology / threads_per_cta -- giving subclasses the entire
        # ``__init__`` tail as a write window.
        self.enable_token_comm: bool = False

        # Warp specialization (lean 8-warp / 256 thread) -- default layout.
        # ``_setup_attributes`` overrides these to the 12-warp MegaMoE
        # layout when ``self.enable_token_comm`` is True; otherwise the
        ...
        # Dispatch / kernel-tail rendezvous NamedBarrier ids exist only
        # under MegaMoE; ``_setup_attributes`` installs them under that
        # mode.  Set to None here so accidental access in lean mode trips
        # an obvious AttributeError instead of silently overlapping with
        # one of the lean barrier ids.
        ...
        # IDs 1-7 are used by the base / epilogue; subclasses that need
        # their own NamedBarriers (e.g. MegaMoE) are free to use IDs
        # 8-15 (the PTX-reserved range), and pick them inside their hook
        # implementations -- the base does not pre-allocate them.
    ...
    def _validate_mma_tiler_and_cluster_shape(self) -> None:
        ...
        if k % sf_k_granularity != 0:
            raise ValueError(
                f"mma_tiler K ({k}) must be a multiple of sf_vec_size * 4 = {sf_k_granularity}"
            )
    ...
    def _setup_attributes(self) -> None:
        ...
        Also the first place to read ``self.enable_token_comm`` -- if set, the
        warp topology / threads_per_cta installed in ``__init__`` is rewritten
        to the 12-warp MegaMoE layout (4 epi + 1 mma + 2 tma + 1 sched + 4
        dispatch) and the two MegaMoE-only NamedBarrier ids are installed.
        Subclasses get the entire ``__init__`` tail (after ``super().__init__``
        returns) as a write window for this flag.
        """
    ...
    def _smem_misc_budget_bytes(self) -> int:
        ...
        SMEM budget.  Base default returns the legacy 1024-byte
    ...
    def get_workspace_size_in_bytes(
        ...
        counter_slots_upper = (data_total_rows + mma_tiler_n -
                               1) // mma_tiler_n + experts
    ...
    def token_comm_hook_dispatch_warp_body(
        ...
        *,
        ...
        """Emitted as the warp body for ``warp_idx >= self.dispatch_warp_id[0]``
        (i.e. warps 8-11 in the MegaMoE 12-warp layout).  Subclass owns
        the entire dispatch flow: prep / cross-rank barrier / per-token
        pull / per-task-tile release-add to the fc1 ready counter / arrive
        on the dispatch->sched NamedBarrier so the sched warp can read
        sizes / etc.  Default: no-op; the base never lands here because
        ``self.enable_token_comm`` is False and the surrounding
        ``const_expr`` guard keeps the branch out of PTX."""
    ...
    def token_comm_hook_kernel_tail(
        ...
        *,
        ...
        """Emitted at the very end of ``fc1fc2_kernel_impl`` on every warp.
        Default: no-op.  MegaMoE: rendezvous all 12 warps on the kernel-tail
        NamedBarrier (so dispatch warps wait until GEMM / combine STG is
        done before firing release-side traffic), then fan out the
        cross-rank NVLink slot=1 release barrier from a designated leader
        warp/lane.  The leader-id choice (``%smid``, CTA linear id,
        ``block_idx().0 == 0``, ...) is the subclass's call -- the base
        does not see it."""
    ...
    def __call__(
        ...
        # MoE-domain layout ``(token_max, topk, hidden)`` BFloat16, hidden
        # stride-1.  Lean fc1+fc2 path: ``topk == 1`` (codegen const), and
        # ``token_max`` is the post-grouping pool token total -- axis 0
        # indexes pool-token rows so each fc2 epi lane writes directly to
        # ``fc2_output[pool_token_global, 0, :]``.  MegaMoE: ``topk ==
        # num_topk``, axis 0 == ``max_tokens_per_rank``; the per-(token,
        # topk) destination is decoded from ``token_comm_args.token_src_
        # metadata`` inside the fc2 epi (see ``Fc2OutputDest``).
        ...
        # ── Sched / runtime ──────────────────────────────────────────────
        # ``offs`` is the cumulative-end variant (sched walks it via
        # ``expert_token_prefix_sum``).  ``expert_token_sizes`` is the
        # per-expert raw count variant (sched walks it via running
        # cumul) and exists for the MegaMoE subclass which exposes a
        # zero-copy view onto ``expert_recv_count_sum``.  Exactly one of
        # the two must be non-None; ``MoEFusedFc12SchedulerParams``
        ...
        # (experts,) Int32 raw token counts (NOT cumulative).  Provide
        # ONLY ONE of ``offs`` / ``expert_token_sizes``.  The MegaMoE
        # subclass routes ``expert_recv_count_sum`` (an i64 tensor) here
        # via a zero-copy ``i32 stride=(2,)`` view -- the same per-expert
        # counts that ``_dispatch_barrier`` finalised inside the same
        # kernel launch.
        expert_token_sizes: Optional[cute.Tensor] = None,
        # ── MegaMoE bundle (Optional) ────────────────────────────────────
        # The lean fc1+fc2 path always passes None; the MegaMoE subclass
        # (``Sm100MegaMoEKernel`` in a separate file) overrides
        # ``__call__`` to build and pass a concrete bundle conforming to
        # the duck-typing contract documented at the top of this file.
        # No type annotation: avoid forward-referencing a class this
        # base file does not (and should not) import.
        ...
        # we do NOT build a GEMM-domain wrapper for it.  The epilogue's
        # ``Fc2UnpackPermuteStg`` consumes it via ``Fc2OutputDest`` and
        # computes per-(token, topk) STG destinations from
        # ``token_comm_args.token_src_metadata`` (or, in lean mode, from
        # the pool-token row index directly).  No sched ext ``"c"`` path
        # in this kernel anymore.
        ...
        fc1_output_sf_gemm_for_fc2_load = cute.make_tensor(
            ...
            blockscaled_utils.tile_atom_to_shape_SF(
                (tokens_sum_padded_sf, intermediate_downproj_padded, c1),
                self.sf_vec_size,
            ),
        )
        ...
        self.fc1fc2_kernel_impl(
            ...
            # kernel forwards it directly to the epilogue's
            # ``Fc2OutputDest`` descriptor.
            fc2_weight_gemm,
            fc2_output,
            fc2_weight_sf_gemm,
            fc1_output_sf_gemm_for_fc2_load,
    ...
    def fc1fc2_kernel_impl(
        ...
        # no GEMM-domain wrapper is built; the epilogue's
        # ``Fc2OutputDest`` consumes the MoE-domain shape directly.
        ...
        # None)`` so when None those branches vanish at codegen time.  See
        # ``fc12_integrate_comm.md`` for the bundle's full field list.
        token_comm_args=None,
    ):
        ...
        num_acc_consumer_threads = len(
            self.epilogue_warp_id) * 32 * (2 if use_2cta_instrs else 1)
        ...
        # legacy "atomic overlaps cluster barrier" timing.  See
        # ``fc12_integrate_comm.md`` §6.
        ...
        # tCtAcc_fake layout: (MMA, MMA_M, MMA_N, STAGE).  Under
            ...
            while work_tile_info.is_valid_tile:
                ...
                if is_phase_linear1:
                    ...
                    tBgB_slice = tBgB[(None, work_tile_info.tile_n_idx, None,
                                       0)]
                    tBgSFB_slice = tBgSFB[(None, work_tile_info.tile_n_idx,
                                           None, 0)]
        ...
        # ── sC SMEM (fc1 NVFP4 output staging; fc2 doesn't use it) ──
        ...
        if warp_idx < self.mma_warp_id:
            ...
            # Acc TMEM tensor passed to the epilogue.  Layout is purely a
            # nominal stand-in -- ``SwapABSwigluFp4Epilogue.run`` only reads
            # ``.iterator`` from this tensor and re-derives all
            # subtile / per-warp lane views internally.
            acc_tensor = cute.make_tensor(
                acc_tmem_ptr,
                _TmemTranspose16x32Core._tmem_layout(
                    128, self.cta_tile_shape_mnk[1]),
            )
