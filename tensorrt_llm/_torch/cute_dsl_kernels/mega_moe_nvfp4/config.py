...
def _num_max_pool_tokens(num_ranks: int, num_tokens_per_rank: int,
                         num_topk: int, num_experts_per_rank: int,
                         block_m: int) -> int:
...
def _num_padded_sf_pool_tokens(num_max_pool_tokens: int, block_m: int,
                               sf_block_m: int) -> int:
...
class DSV4Config:
    ...
    def num_padded_sf_pool_tokens(self) -> int:
        return _num_padded_sf_pool_tokens(self.num_max_pool_tokens,
                                          self.block_m, self.sf_block_m)
...
def transform_sf_token_idx_numpy(token_idx_in_expert):
    ...
    return ((t // np.int32(DSV4.block_m)) * np.int32(DSV4.sf_block_m) +
            (idx & np.int32(-128)) + (idx & np.int32(31)) * np.int32(4) +
            ((idx >> np.int32(5)) & np.int32(3))).astype(np.int32)
