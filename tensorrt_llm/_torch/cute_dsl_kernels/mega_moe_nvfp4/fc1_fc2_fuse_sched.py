...
class MoEFusedFc12SchedulerParams(MoESchedulerParamsBase):
    ...
    def __new_from_mlir_values__(
        ...
        assert idx == len(values), (
            f"Fused fc12 sched params type-discrim mismatch: idx={idx} len(values)={len(values)}"
        )
        return result
...
class MoEFusedFc12PersistentTileScheduler(MoESchedulerBase):
    ...
    def internal_init(
        ...
        *,
        ...
        if const_expr(self.params.load_balance_mode == "atomic_counter"):
            cluster_size = self.params.cluster_shape_mn[
                0] * self.params.cluster_shape_mn[1]
    ...
    def _advance_work_linear_tile_idx_static(
        ...
        *,
        ...
        state.current_work_linear_tile_idx = cluster_linear_tile_idx + self.num_persistent_clusters
    ...
    def _advance_expert_within_phase(
        ...
        *,
        ...
        if state.current_phase == Int32(BlockPhase.Linear1):
            tiles_in_expert = state.current_token_block_count * self._num_fc1_intermediate_blocks
        else:
            tiles_in_expert = state.current_token_block_count * self._num_fc2_hidden_blocks
        state.current_expert_tile_end = state.current_expert_tile_start + tiles_in_expert
    ...
    def _advance_group(
        ...
        *,
        ...
        while expert_cursor < self.expert_cnt and cumulative_fc1 < threshold:
            ...
            cumulative_fc2 = cumulative_fc2 + token_block_count_e * self._num_fc2_hidden_blocks
        ...
        state.current_group_end = state.current_group_fc1_subphase_end + group_total_fc2_tiles
    ...
    def _decode_inside_expert(
        ...
        *,
        ...
        remaining_in_expert = state.current_this_expert_token_cnt - token_idx_start_in_expert
    ...
    def _gen_work_from_cluster_idx(
        ...
        *,
        ...
        while (cluster_linear_tile_idx >= outer_group_end
            ...
            self._fused_state = self._fused_state  # DSL carry
            state = self._fused_state  # re-bind alias inside body so refresh below sees new SSA
            outer_group_end = state.current_group_end
            outer_group_last_expert_exclusive = state.current_group_last_expert_exclusive
